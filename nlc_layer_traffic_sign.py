...
for noisy_ratio in noisy_ratio_lst:
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            labels_changed = torch.tensor(labels_changed, dtype=torch.long).to(device)
        ...
        if (epoch + 1) % 2 == 0:
            ...
            with torch.no_grad():
                ...
                for testimages, testlabels in test_loader:
                    ...
                    testlabels_changed = torch.tensor(testlabels_changed, dtype=torch.long).to(device)
    ...
    pd.DataFrame(loss_all).to_csv("nlc_layer_resnet34_traffic_sign_sy_" + str(noisy_ratio) + ".csv")
