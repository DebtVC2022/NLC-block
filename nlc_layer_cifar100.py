...
#  训练数据集
train_dataset = torchvision.datasets.CIFAR100(root=cifar10Path, train=True, transform=transform, download=True)

# 测试数据集
test_dataset = torchvision.datasets.CIFAR100(root=cifar10Path,train=False, transform=transform)
...
class NoisyLabelCorrectionLayer(nn.Module):
    ...
    def forward(self, y_hat_i, y_i):
        y_pred_i = torch.max(y_hat_i, 1).values

        exponent = y_pred_i
        denominator = 1 + y_i * y_pred_i  # Corrected the denominator
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        num_samples_to_change_train = int(0.1 * len(labels_np))
    ...
    if (epoch + 1) % 2 == 0:
        ...
        with torch.no_grad():
            ...
            for images, testlabels in test_loader:
                ...
                num_samples_to_change_testlabels = int(0.1 * len(testlabels_np))
                ...
                outputs = model(images, testlabels_changed)
